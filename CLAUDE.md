...
- **Pipeline validation = evidentiary run.** Whenever validating a released
  ...
  previously-FAILed artifacts after gate patches, hostile-precondition case,
  claim ceiling). Agent-triggered by this rule — the user should never need
  to type it. Record usage: `python3 skills/learn-skill/tools/telemetry.py
