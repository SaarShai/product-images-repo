...
- **Image output location.** ALWAYS save/copy result images to an `Images/` subfolder inside
  the folder the user points to — typically the production folder where the `.ai`/`.svg`
  template lives (e.g. `…/production files/<product>/Images/`), as `Images/finals/` +
  `Images/candidates/`. Repo `tasks/<task>/outputs/` is the working copy only. (user rule, 2026-06-24)
