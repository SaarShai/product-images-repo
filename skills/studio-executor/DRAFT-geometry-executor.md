...
13. **EXECUTION-SIM FIX (2026-07-05): panel-scoped gating for single panels of a multi-panel template.** `objective_gate_report.py --cand <panel> --svg <full-template.svg>` FAILS by construction (aspect mismatch whole-SVG vs one panel + missing contour-mask.png) — proven in execution sim. For a single panel use the panel-scoped pair: `python3 -m studio.controlmap --score <cand> --mask <panel>-mask.png` (shape) AND `python3 scripts/geom_gate.py --cand <cand> --mask <panel>-mask.png` (NO --svg = mask-authoritative mode; gates fill_inside_contour, so it also catches near-empty panels deterministically). Reserve objective_gate_report.py for single-panel-SVG tasks.
