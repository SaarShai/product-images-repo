...
7. **PROVEN ONE-PASS RECIPE (2026-07-05 — this is now the DEFAULT route for die-cut panels with a trained LoRA; supersedes the LoRA-vs-IP-Adapter branch above whenever a LoRA exists).** Validated end-to-end on Cap Juluca (easy) AND the Marriott 3-panel city-skyline (hard, no drawn SVG contour). Geometry and full style come out in ONE generation — no separate structure pass.
   - **Endpoint:** direct `fal_client.subscribe("fal-ai/flux-control-lora-canny", {...})` — NOT a falgen.py flag (see amendment 4). Args that worked: `control_lora_image_url` = uploaded `<panel>-control.png`; `control_lora_scale` 0.35 (this is the geometry↔style dial — raise toward 0.6 for tighter geometry, lower for looser/more painterly); `loras=[{"path": <lora_url>, "scale": 1.05}]`; `guidance_scale` 4.0; `num_inference_steps` 40; `image_size` = the control-map dims; `enable_safety_checker` False; `num_images` ≥3 for best-of-N.
   - **Control map is authored, not prompted:** build it with `python3 -m studio.controlmap --spec <panel>.spec.json [--guide <guide.png>] [--content edges.json] --outdir <dir>`. `--guide` mode is mandatory for city-skyline narrows (the SVG carries no drawn narrow contour). Content that MUST appear (an arch gateway, a window) goes in `--content` as white edges — LAW 0 for the control channel; do NOT rely on the prompt to place fixed structure.
   - **Blank-plaque / no-text is a HARD prompt rule (memory `no-painted-text-vector-layers`):** every prompt renders signs/plaques/banners as BLANK placards (shape+color, no glyphs) and carries the suffix clause "all signs, plaques and banners are BLANK with no letters, no words, no writing, no glyphs of any kind". Text is added later as .ai vector type. Never prompt a sign "reading X".
   - **TWO-GATE acceptance, both required (neither alone is sufficient):** (a) deterministic silhouette-IoU ≥0.85 via `studio.controlmap.panel_silhouette` on the near-white-flooded candidate vs `<panel>-mask.png`; (b) Fable/VLM vision judge for felt style + blank signage + required content. PROVEN necessity: Marriott `right_s1` was a near-EMPTY panel yet scored IoU 0.976 — the vision gate is what rejected it. IoU confirms shape, never content (memory `region-iou-not-fit-calibration`).
   - **fal bucket drift:** fal snaps `image_size` to the nearest allowed bucket (narrows requested 820×2105 came back 576×1536 = aspect 0.375 vs 0.3895; door 800×1184). Measure IoU after resizing the candidate to the mask size, and absorb the drift by placing finals into the spec bbox at assembly. (Open: a bucket-normalization/upscale-to-exact step.)
   - **Finish chain (per winner):** `scripts/reupscale.py --creativity 0.5 --resemblance 0.6 --factor 2` (clarity re-upscale, sweet spot per memory `reupscale-fixes-magnific-defects`) → `scripts/dehalo.py` (kills the clarity grey edge-halo) → `scripts/white_key.py` (clean white→transparent cutout). Then assemble panels at their true `bbox_svg` positions in the shared `viewbox` (all panels of one template share it).
   - **Cross-panel consistency (open gap):** independent per-panel gens drift in style (Marriott center read more 3D-relief, sides flatter). Tighten by feeding one approved panel as a style reference for the others (memory `reference-lock-for-consistency`) before finishing.

## DRY-RUN FIXES (2026-07-05 — binding; from a gpt-5.5 executor dry-run, verified against actual code)

8. **Amendment 7 is now EXECUTABLE — use the wrapper, not raw fal_client:** `python3 scripts/onepass_gen.py --control <panel>-control.png --lora-json <lora.json> --prompt "<TRIGGER> ..." --width <W> --height <H> --out-prefix <dir>/rN_<panel> [-n 3] [--control-scale 0.35] [--lora-scale 1.05] [--mask <panel>-mask.png] [--dry-run]`. It uploads the control map, validates lora.json is COMPLETED, REFUSES a prompt missing the trigger word, auto-appends the blank-signage clause (skip only with `--allow-text`), and with `--mask` scores every output inline. ALWAYS run `--dry-run` first (validates + prints the exact request, zero spend).
9. **Gate (a) is now a CLI:** `python3 -m studio.controlmap --score <cand.png> --mask <panel>-mask.png` → JSON `silhouette_iou`/`shape_pass` (resize-normalized for fal bucket drift). Gate (b) today = `python3 scripts/judge.py --mode check --image <cand> --criterion "..."` (single OpenAI gpt-4o — NOT Fable; see amendment 3) for felt style + blank signage + required content, OR a human/Fable review of the contact sheet. Both gates mandatory; run (a) first, it's free.
10. **Real CLIs (body's invented flags are void):** `build_reference_style_packet.py <task_dir> --refs <r1 r2 ...>` (positional task dir; no `--collection`/`--output_dir`). `judge.py --mode pairwise --image <A> --ref <B> --criterion "<c>"` (no `--image_a`/`--judges`/`--rubric`/`--output`). `dehalo.py --image <in> --out <out>` and `white_key.py --image <in> --out <out>` (both REQUIRE `--out`). `reupscale.py --image <in> --out <out> --creativity 0.5 --resemblance 0.6 --factor 2` (verified flags).
11. **Assembly:** place each finished RGBA panel at its `bbox_svg` position scaled from the shared `viewbox` (all panels of one template share it — read both from the spec.jsons). Reference implementation: `scratchpad/assemble_r3.py` pattern (Marriott r3-screen-preview) — to be promoted to `studio/assemble.py`.
12. **LoRA registry:** per-collection `lora.json` files (`.brainer/tenx/lora-pilot/lora.json` = CJWC, `.brainer/tenx/marriott-lora/lora.json` = MRCH). Usable iff `status == "COMPLETED"` and `lora_url` present — `onepass_gen.py` enforces this. The id `fal:txcl_watercolor_lora_v2` does not exist anywhere; never cite it.
13. **EXECUTION-SIM FIX (2026-07-05): no-spend mode is a first-class path.** When validating a pipeline (sim, CI, skill dry-run): generation = `onepass_gen.py --dry-run` (prints exact request, zero spend); judge.py = SKIP and record `"judge": "SKIPPED (no-spend)"` in the report — the run is then explicitly NOT acceptance-complete. Deterministic gates (controlmap --score, geom_gate --mask) always run — they are free.
14. **STYLE-JUDGING PROTOCOL + REF-ANCHORED RESTYLE (2026-07-05, proven Marriott r3→r5).** (a) Style verdicts require a COMPARISON BOARD: refs row + each candidate round row, same scale — never judge from memory or single contact sheets (r4 lesson: texture-biased call missed a washed-out palette). (b) Restyle prompts must NOT describe the style in prose — describing "dense wool-felt, 3D relief" caused description-anchored drift (LAW 0 applies to the restyle channel). Correct: `falgen.py --mode flux2edit --image <cand> --refs <ACTUAL ref image>` + minimal prompt "Repaint in EXACTLY the reference's art style... keep composition/framing exactly, all signs blank". (c) Geometry after restyle = deterministic MASK-CUT aperture lock: resize `<panel>-mask.png` to candidate, force outside→white, then gate (`--score` + `geom_gate --mask`); restyle from a FULL-BLEED source (art reaching all edges) or fill_inside drops. Result: IoU 0.985-0.998 at ref-level style. (d) Watch COLOR PULL: the ref's palette can override the source design's colors (white hospital → navy from Police ref) — flag to the human arbiter; aesthetics are never self-judged final.
