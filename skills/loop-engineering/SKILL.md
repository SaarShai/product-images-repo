...
- [`../_shared/model_roster.py`](../_shared/model_roster.py) — detect reachable cross-vendor backends; render read-only advisor (diverge) / verifier (converge) dispatches. Shared with [`verify-before-completion`](../verify-before-completion/SKILL.md).