#!/usr/bin/env bash
# run_round3.sh — 2 arms x 3 runs, white bg, openai provider. Tests the CORE-swap lever.
...
LOG='tasks/transparent-clear-edge-claude/run_round3.log'
mkdir -p "$OUT"
: > "$LOG"
job() {
  arm="$1"; run="$2"
...
printf "%s\n" \
  "R3a-avoid-thin 1" "R3b-keyable-fine 1" "R3a-avoid-thin 2" \
  "R3b-keyable-fine 2" "R3a-avoid-thin 3" "R3b-keyable-fine 3" \
