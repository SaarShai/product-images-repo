#!/usr/bin/env bash
# run_round2.sh — 2 arms x 3 runs, white bg, openai provider. Tests the CORE-swap lever.
...
LOG='tasks/transparent-clear-edge-claude/run_round2.log'
mkdir -p "$OUT"
: > "$LOG"
job() {
  arm="$1"; run="$2"
...
printf "%s\n" \
  "R2a-refauthority 1" "R2b-m1-densitynudge 1" "R2a-refauthority 2" \
  "R2b-m1-densitynudge 2" "R2a-refauthority 3" "R2b-m1-densitynudge 3" \
| xargs -P 3 -L 1 bash -c 'job "$@"' _
echo "ROUND2 COMPLETE $(date +%T)" >> "$LOG"
