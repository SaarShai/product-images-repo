...
class Params:
    ...
    erase_dilate: int = 3
    # Process at this max side for LaMa (full x4 is heavy); upsample paper back.
    ...
    tile_overlap: int = 64  # unused if single-pass fits
...
def build_erase_mask(rgb: np.ndarray, p: Params) -> tuple[np.ndarray, np.ndarray, dict[str, Any]]:
    """erase=True where LaMa may rewrite; outer paper locked (erase=False)."""
    ...
    # Approximate art: not outer paper (includes enclosed pale pockets + paint)
    art = ~outer_paper
    erase = art.copy()
    if p.erase_dilate > 0:
        erase = ndi.binary_dilation(erase, iterations=p.erase_dilate)
    # Hard lock: never erase true outer paper
...
def run_lama(
    ...
    """Inpaint erase region; copy original outside erase (mask lock)."""
    h, w = rgb.shape[:2]
    ...
    # Upsample paper estimate to full res
    if scale < 1.0:
        paper = np.asarray(
    ...
    # Mask lock: restore original outside erase
    locked = paper.copy()
...
def delta_alpha(
    ...
    """Soft alpha from |src-paper|; optional paper-subtract on rim."""
    ...
    # Outside erase (true paper lock): force transparent
    soft[outer_paper] = 0.0
    # Pixels never erased and near paper: also transparent if delta tiny
    soft[(~erase) & (delta < p.delta_thr)] = 0.0

    # Optional rim paper-subtract: near erase boundary, kill near-paper leftovers
    ...
    if count:
        ...
        # restore soft values where soft was mid but survived? already multiplied

    # Unblend partials toward paint (avoid white halo)
    mean, _ = paper_model(rgb)
    alpha = soft.copy()
