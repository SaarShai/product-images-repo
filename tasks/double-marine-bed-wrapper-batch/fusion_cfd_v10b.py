#!/usr/bin/env python3
"""v10b Semantic-Skeleton CFD with WIDE unknown band.

v10 failure: marking ALL near-paper as sure BG collapsed unknown to ~0.1%,
so CF never matted and white rim survived.

v10b fix:
  Sure FG  = chromatic skeleton (chroma >= T) dilated modestly
             + optional BRIA sure-FG eroded; NEVER paper as FG.
  Sure BG  = ONLY border-connected pure paper (flood from image border).
             Enclosed near-white between branches stays UNKNOWN.
...
class Params:
    ...
    chroma_dilate_px: int = 4
    paper_luma_min: float = 248.0
    paper_chroma_max: float = 4.0
    # Optional: also mark pure paper that is far from chromatic paint as BG
    # (keeps enclosed paper near paint in unknown). 0 = disabled.
...
def border_connected(mask: np.ndarray) -> np.ndarray:
    ...
    if not border_labels:
        return np.zeros_like(mask, dtype=bool)
    keep = np.isin(labeled, list(border_labels))
    return keep
...
def build_trimap(rgb: np.ndarray, bria: np.ndarray | None, p: Params) -> np.ndarray:
    ...
    # Sure FG: chromatic skeleton, modest dilate; never claim paper.
    ...
    if bria is not None and p.use_bria:
        ...
        # Never let BRIA claim paper as FG
        fg |= bria_sure & ~paper

    # Sure BG: ONLY border-connected pure paper.
    bg = border_connected(paper)

    # Optional: enclosed pure paper far from chromatic paint → BG
    # (keeps near-paint enclosed paper in unknown for CF).
    if p.enclosed_paper_clearance_px > 0:
        chromatic = chroma >= p.chroma_fg_min
        dist = ndi.distance_transform_edt(~chromatic)
        far_paper = paper & (dist >= p.enclosed_paper_clearance_px) & ~fg
        bg |= far_paper

    # Conflicts: prefer FG on chromatic, else BG
    both = fg & bg
    if both.any():
        fg[both] = chroma[both] >= p.chroma_fg_min
        bg[both] = ~fg[both]

    # Ensure FG and BG don't leave zero unknown by accident — remaining is unknown
...
def run_pipeline(
    ...
    print(f"[trimap] fg={(trimap==1).mean()*100:.3f}% bg={(trimap==0).mean()*100:.3f}% unk={unk:.3f}% ({t_trimap:.1f}s)")
...
def upsample_rgba(rgba: np.ndarray, size: tuple[int, int]) -> np.ndarray:
    """Nearest for alpha, LANCZOS for RGB where opaque."""
...
def main() -> None:
    ...
    ap.add_argument("--bria-erode", type=int, default=6)
    ap.add_argument("--clearance", type=int, default=0, help="enclosed paper clearance px")
    ap.add_argument("--flatten", type=float, default=0.15)
    ap.add_argument("--full-max", type=int, default=3600)
    ap.add_argument("--half", action="store_true", help="run CF at half-res then upsample")
    ap.add_argument("--tag", type=str, default="v10b-wideunk")
    ap.add_argument("--prefix", type=str, default="18-cfd-v10b")
    args = ap.parse_args()
    ...
    if p.half_res:
        ...
        # Scale morphology params roughly for half-res
        p_run = Params(**{**asdict(p), "chroma_dilate_px": max(1, p.chroma_dilate_px // 2),
    ...
    # Trimap visualization (half-size for review)
    tvis = np.zeros((*trimap_full.shape, 3), dtype=np.uint8)
    tvis[trimap_full == 0.0] = (0, 0, 255)  # BG blue
    tvis[trimap_full == 1.0] = (0, 255, 0)  # FG green
    tvis[np.isclose(trimap_full, 0.5)] = (128, 128, 128)  # unknown gray
    # Also catch any other unknown-ish
    ...
    # ROI trimap crops for verification
