...
Idea:
  ...
  2. Grow seeds into anything that is NOT pure paper (geodesic dilate).
...
class Params:
    ...
    # Pure paper = only non-growable / BG-walkable
    pure_chroma_max: float = 2.0
    pure_luma_min: float = 252.0
    grow_iters: int = 0  # 0 = full reconstruction until stable
...
def geodesic_dilate(seeds: np.ndarray, allowed: np.ndarray, max_iters: int = 0) -> np.ndarray:
    """Dilate seeds only inside allowed mask until stable (or max_iters)."""
    out = seeds & allowed
    struct = ndi.generate_binary_structure(2, 1)
    it = 0
    while True:
        grown = ndi.binary_dilation(out, structure=struct) & allowed
        if not np.any(grown & ~out):
            break
        out = grown
        it += 1
...
def run_pipeline(rgb: np.ndarray, p: Params) -> tuple[np.ndarray, dict[str, Any]]:
    ...
    allowed = ~pure_paper  # growable: anything with paint signal

    seeds = (chroma >= p.seed_chroma_min) | (luma <= p.seed_luma_max)
    seeds &= allowed

    grown = geodesic_dilate(seeds, allowed, max_iters=p.grow_iters)
    fg = fill_holes(grown)
...
def make_review(path: Path, rgba: np.ndarray, x: int, y: int, w: int, h: int, scale: int = 2) -> None:
    ...
    # Left panel: composite on white so transparent shows as white (not leftover RGB)
